// Monte Carlo batch runner - runs a range of seeds and appends to a shared results file
// Usage: node montecarlo-batch.js <startSeed> <count> <timeout> <outputFile>
...
const OUTPUT_FILE = process.argv[5] || 'montecarlo-batch-results.json';
...
for (let i = 0; i < COUNT; i++) {
    ...
    if (done <= 3 || done % 20 === 0 || done === COUNT) {
        ...
        process.stdout.write(`\r  Batch seeds ${START_SEED}-${START_SEED + COUNT - 1}: ${done}/${COUNT} | ${wins}W/${losses}L/${timeouts}T | ${totalTime}s   `);
    }
}
